...
// 				// sprintf(message, "top_rank=%d, bottom_rank=%d, left_rank=%d, right_rank=%d",
// 				// 		alert_message.top_rank, alert_message.bottom_rank, alert_message.left_rank, alert_message.right_rank);
// 				// log_charging_node_event(node, message);

// 				// sprintf(message, "top availability=%d, bottom availability=%d, left availability=%d, right availability=%d",
// 				// 		top_availability, bottom_availability, left_availability, right_availability);
...
// 				// MPI_Recv(&available_nodes, 1, MPI_INT, BASE_STATION_RANK, ALERT_TAG, node->world_comm, MPI_STATUS_IGNORE);
